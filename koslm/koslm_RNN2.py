...
class KOSLMBlock(nn.Module):
    def __init__(self, config, segment_len=48, use_ckpt=False):
        super().__init__()
    ...
    def _segment_scan(self, x_seg, h0, c0):
        ...
        if self.use_ckpt and self.training:
            h, c = checkpoint(lambda _x, _h, _c: self.cell(_x, (_h, _c))[:3],
                                 x_seg, h, c, use_reentrant=False)
        else:
            h, c = self.cell(x_seg, (h, c))
        return h, c
    ...
    def forward(self, x):
        ...
        device = x.device
        self.segment_len = 24
        h = torch.zeros(B, self.segment_len, self.H, device=device)
        c = torch.zeros(B, self.segment_len, self.H, device=device)
        outputs = []
        for s in range(0, L, self.segment_len):
            seg = x[:, s:s+self.segment_len, :]
            h, c = self._segment_scan(seg, h, c)
            outputs.append(h)
...
class KOSLMSSM(nn.Module):
    ...
    def forward(self, gates_2H: torch.Tensor, c_prev: torch.Tensor):
        ...
        z_t = torch.tanh(z_raw + self.z_bias)                              # [B,L, H]
        ...
        B_tz_t = B_t * z_t
        c_t = pscan(A_t.unsqueeze(-1), B_tz_t.unsqueeze(-1)).squeeze(-1)  # + 0.5 * FiLMBlock(x)
