...
from Efficiency_Benchmark import benchmark_efficiency
...
def train_model(model, train_loader, val_loader, criterion, optimizer, scheduler, num_epochs, device, min_lr=0.00001, patience=1):
    """
    训练模型并添加验证逻辑。
...
current_dir = os.path.dirname(os.path.abspath(__file__))  # 获取当前
...
def model_selective(modelSelection, num_features):
    ...
    model_dict = {
        ...
        "KOSLM": ("KOSLMDefiniteSix", "paramsConfig", "KOSLMModel"),
    }
    ...
    elif modelSelection == "KOSLM":
        model_kwargs.update({
            "KOSLM_hidden_size": params["KOSLM_hidden_size"],
            "KOSLM_num_layers": params["KOSLM_num_layers"],
            "observation_level_size": params["observation_level_size"],
        })
...
def make_dummy(batch, seq_len, d_model, device, dtype):
    x = torch.randn(batch, seq_len, d_model, device=device, dtype=dtype)
    y = torch.randn(batch, 1, d_model, device=device, dtype=dtype)
    return x, y

def get_nvidia_memory():
    # 返回单位 GB（总 GPU 已用），如果系统没有 nvidia-smi，返回 None
    try:
        out = subprocess.check_output(
            ["nvidia-smi", "--query-gpu=memory.used", "--format=csv,noheader,nounits"],
            stderr=subprocess.DEVNULL
        ).decode().strip().splitlines()
        # 取第一个 GPU 的值（单 GPU 实验常用）
        val_mb = float(out[0])
        return val_mb / 1024.0
    except Exception:
        return None

def training_peak_mem(model, batch, seq_len, d_model, device="cuda", dtype=torch.float16,
                      warmup=3, iters=5, use_amp=True):
    model = model.to(device)
    model.train()
    opt = torch.optim.AdamW(model.parameters(), lr=1e-3)
    loss_fn = nn.MSELoss()

    amp_ctx = (torch.autocast(device_type="cuda", dtype=dtype) if (use_amp and device=="cuda") else nullcontext())

    # warmup to initialize optimizer state (and any lazy allocations)
    for _ in range(warmup):
        x, y = make_dummy(batch, seq_len, d_model, device, dtype if device=="cuda" else torch.float32)
        opt.zero_grad(set_to_none=True)
        with amp_ctx:
            out = model(x)
            # accommodate LSTM that returns (output, (h,c))
    ...
    for _ in range(iters):
        ...
        torch_peak_gb.append(torch_peak)
        nvidia_peak_gb.append(nvidia_mem if nvidia_mem is not None else float('nan'))

    # return median
    torch_peak_gb = sorted(torch_peak_gb)
    nvidia_peak_gb = sorted(nvidia_peak_gb)
    mid = len(torch_peak_gb)//2
    return torch_peak_gb[mid], (nvidia_peak_gb[mid] if not any([v!=v for v in nvidia_peak_gb]) else None)

# --------- 你的模型 factory（替换以下占位） ----------
def build_lstm(d_model=256, n_layers=4):
    return nn.LSTM(input_size=d_model, hidden_size=d_model, num_layers=n_layers, batch_first=True)

def build_transformer(d_model=256, n_layers=4, n_heads=4):
...
def build_mamba(d_model=256, n_layers=4):
    num_features = d_model
    _, model, _ = model_selective("Mamba", num_features)
    return model

def build_koslm(d_model=256, n_layers=4):
    num_features = d_model
    _, model, _ = model_selective("KOSLM", num_features)
    return model
# ----------------------------------------------------

def wrap_with_heads(backbone, d_model=256):
...
if __name__ == "__main__":
    ...
    seq_len = 2048       # 论文里对齐的长度
    d_model = 256
    n_layers = 4
    batch_list = [1,2,4,8,16,32,64,128]

    configs = [
        ("LSTM", lambda: wrap_with_heads(build_lstm(d_model, n_layers), d_model)),
        ("Mamba", lambda: wrap_with_heads(build_mamba(d_model, n_layers), d_model)),
        ("KOSLM", lambda: wrap_with_heads(build_koslm(d_model, n_layers), d_model)),
        ("Transformer", lambda: wrap_with_heads(build_transformer(d_model, n_layers, n_heads=4), d_model)),
    ]

    # CSV 导出表头
    with open("efficiency_train_results.csv", "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["model","batch","params_M","torch_peak_GB","nvidia_smi_GB"])
        for name, builder in configs:
            try:
                ...
                continue
            print(f"Measuring {name}, params={params/1e6:.2f}M")
            for b in batch_list:
                try:
                    tpeak, npeak = training_peak_mem(model, b, seq_len, d_model, device=device,
                                                     dtype=torch.float16, warmup=3, iters=5, use_amp=True)
                except RuntimeError as e:
                    print(f"{name} batch {b} -> OOM / error: {e}")
                    tpeak, npeak = float('nan'), float('nan')
                print(f"  batch {b}: torch_peak={tpeak:.3f} GB, nvidia_smi~{npeak}")
                writer.writerow([name, b, f"{params/1e6:.3f}", f"{tpeak:.3f}", f"{npeak if npeak is not None else ''}"])
    print("done -> efficiency_train_results.csv")
...
#     # _, model_transformer, device = model_selective("Transformer", num_features)
#     # _, model_mamba, device = model_selective("Mamba", num_features)
#     # _, model_lstm, device = model_selective("LSTM", num_features)
#     # _, model_koslm, device = model_selective("KOSLM", num_features)
#     #
#     # models = {
#     #     "Transformer": model_transformer.to(device),
#     #     "Mamba": model_mamba.to(device),
#     #     "LSTM": model_lstm.to(device),
#     #     "KOSLM": model_koslm.to(device),
#     # }
#     #
#     # benchmark_efficiency(models, dataset_path, L_list=[512, 1024, 2048, 4096, 8192, 16384], B=8, D=num_features, amp=True)
#     #############################benchmark_efficiency################################
#
